...
private:
  ...
  ///Parameters 通过程序开始运行时读入parameters.yml文件进行初始化 
  int bbox_step;
  int min_win;
  int patch_size;
  int scale;//上下文空间对初始框的倍数
  cv::Rect firstbox;

  //initial parameters for positive examples
  //从第一帧得到的目标的bounding box中（文件读取或者用户框定），经过几何变换得  
  //到 num_closest_init * num_warps_init 个正样本
  int num_closest_init;//最近邻窗口数10
  int num_warps_init;//几何变换数目20
  int noise_init;
  float angle_init;
  float shift_init;
  float scale_init;
...
public:
  ...
  int n;
  cv::Point center;
  cv::Rect cxtRegion_last;
  cv::Rect cxtRegion_current;
  void read(const cv::FileNode& file);
  //Methods
  //void init(const cv::Mat& frame1,const cv::Rect &box, FILE* bb_file);
  void init(const cv::Mat& frame,cv::Rect &box, FILE* bb_file);
  ...
  void processFrame(const cv::Mat& img3,cv::Mat& img4,std::vector<cv::Point2f>& points1,std::vector<cv::Point2f>& points2,
       BoundingBox& bbnext,bool& lastboxfound, bool tl,FILE* bb_file);
