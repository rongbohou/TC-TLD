...
void TLD::init(const Mat& frame,Rect& box,FILE* bb_file){
  ...
  // the object position
	...
	// the context region
	cxtRegion_last.width = scale* box.width;
	cxtRegion_last.height = scale* box.height;
	cxtRegion_last.x = center.x - cxtRegion_last.width *0.5;
	cxtRegion_last.y = center.y - cxtRegion_last.height *0.5;
	cxtRegion_last &= Rect(0, 0, frame.cols, frame.rows);//在原始frame中提取cxtRegion
	 cout<<"cxtRegion_last.x"<<cxtRegion_last.x<<"  "<<"cxtRegion_last.y"<<cxtRegion_last.y <<"  "<<"cxtRegion_last.width"<<cxtRegion_last.width<<"  "<<"cxtRegion_last.height"<<cxtRegion_last.height<<endl;
	//边缘处理
	 cout<<"边缘处理后"<<endl;
	 if(cxtRegion_last.width<scale*box.width||cxtRegion_last.height<scale*box.height)
	{
		cxtRegion_last.width=scale*box.width;
		cxtRegion_last.height=scale*box.height;
		if((cxtRegion_last.x+cxtRegion_last.width)>frame.cols)
			cxtRegion_last.x=frame.cols-cxtRegion_last.width;
		if((cxtRegion_last.y+cxtRegion_last.height)>frame.rows)
			cxtRegion_last.y=frame.rows-cxtRegion_last.height;
		
	} 
	cout<<"cxtRegion_last.x"<<cxtRegion_last.x<<"  "<<"cxtRegion_last.y"<<cxtRegion_last.y <<"  "<<"cxtRegion_last.width"<<cxtRegion_last.width<<"  "<<"cxtRegion_last.height"<<cxtRegion_last.height<<endl;
	cxtRegion_current=cxtRegion_last;

	Mat frame1=frame(cxtRegion_last);
	box.x=box.x-cxtRegion_last.x;
	box.y=box.y-cxtRegion_last.y;
  //Get Bounding Boxes
 //根据传入的box（目标边界框）在传入的图像frame1中构建全部的扫描窗口，并计算重叠度
	firstbox.width=box.width;
	firstbox.height=box.height;
 ...
 * -num_warps(图像块变换次数)
 * Outputs:
 * - Positive fern features (pX)集成分类器正样本
...
//frame(best_box)为该帧图像中best_box边界框对应图像块；将该图像块resize为15*15的大小；
//计算该图像的均值、方差，分别存放到mean、stdev中；将该图像块与均值作差的结果存于pEx中。
//Get Fern features on warped patches
  ...
  for (int i=0;i<num_warps;i++){
     if (i>0)
       //generator(frame,pt,warped,bbhull.size(),rng);// pt为good_boxes最大边界框（2中有提到）的中心点；该函数对每个good_box对应图像块仿射变换：±1%范围的偏移，
       generator(img,pt,img,frame.size(),rng);     //±1%范围的尺度变化，±10%范围的平面内旋转，并且在每个像素上增加方差为5的高斯噪声（确切的大小是在指定的范
                                                   // 围内随机选择的），共计num_warps_init种。对每一仿射图像块计算特征，并存于pX中。
...
void TLD::processFrame(const cv::Mat& img3,cv::Mat& img4,vector<Point2f>& points1,vector<Point2f>& points2,BoundingBox& bbnext,bool& lastboxfound, bool tl, FILE* bb_file)
{ 
  ...
  Mat img1,img2;
  ///Track
    img1=img3(cxtRegion_last);
    img2=img4(cxtRegion_current);
   drawBox(img4,cxtRegion_current);
  if(lastboxfound && tl){
  ...
  //update trackBox and context region
  //update center
  
if(tracked)
  {
	//if(cxtRegion_current!=Rect(0, 0, img4.cols, img4.rows))
	//{
	center.x = bbnext.x + 0.5 * bbnext.width+cxtRegion_current.x;
	center.y = bbnext.y + 0.5 * bbnext.height+cxtRegion_current.y;

	swap(cxtRegion_last,cxtRegion_current);

  //update trackBox
	bbnext.width=firstbox.width;
	bbnext.height=firstbox.height;
	bbnext.x=center.x-0.5*bbnext.width;
	bbnext.y=center.y-0.5*bbnext.height;
	bbnext &= Rect(0, 0, img4.cols, img4.rows);

  //update context region (边缘处理)
	cxtRegion_current.x = center.x - cxtRegion_current.width *1/2;
	cxtRegion_current.y = center.y - cxtRegion_current.height *1/2;
    cxtRegion_current &= Rect(0, 0, img4.cols, img4.rows);
	if(cxtRegion_current.width<scale*firstbox.width||cxtRegion_current.height<scale*firstbox.height)
	{
	
		cxtRegion_current.width=scale*firstbox.width;
		cxtRegion_current.height=scale*firstbox.height;
		if((cxtRegion_current.x+cxtRegion_current.width)>img4.cols)
			cxtRegion_current.x=img4.cols-cxtRegion_current.width;
		if((cxtRegion_current.y+cxtRegion_current.height)>img4.rows)
			cxtRegion_current.y=img4.rows-cxtRegion_current.height;
  ...
  { 
	 center=Point(0,0);
	 drawBox(img4,cxtRegion_current);
	 cxtRegion_last=cxtRegion_current;
  }

  t = (double)cvGetTickCount() - t;
  cout << "precess cost  " << t / ((double)cvGetTickFrequency()*1000.)<<"ms" << endl;
}
...
void TLD::track(const Mat& img1, const Mat& img2,vector<Point2f>& points1,vector<Point2f>& points2){
  ...
  //t = (double)cvGetTickCount() - t;
  //cout << "LK tracker cost  " << t / ((double)cvGetTickFrequency()*1000.)<<"ms" << endl;
  }
  else
...
void TLD::detect(const cv::Mat& frame){
  ...
  //如果通过以上两个检测模块的扫描窗口数大于50个，则只取后验概率大的前50个
  if (detections>100)
  {
...
//  imshow("detections",img);
  ...
  o = (double)cvGetTickCount() - o;
  cout << "detecting cost  " << o / ((double)cvGetTickFrequency()*1000.)<<"ms" << endl;
}
...
/// Data generation
  ...
  classifier.show();
  t = (double)cvGetTickCount() - t;
  cout << "Learning cost  " << t / ((double)cvGetTickFrequency()*1000.)<<"ms" << endl;
}

/****input:current image、init box;output: vector<BoundingBox> grid ****/
...
/*******intput:init box、the number of good box that would get********
    ****output:best box、vect<int>good_box、vect<int>bad_box*/
